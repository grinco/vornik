...
vornik reads its configuration from `config.yaml`. The keys below are the customer-facing settings, by dotted YAML path. Where an environment variable overrides a key it is listed; an override wins over the file, and `vornikctl config show --provenance` shows which one supplied each value on the running daemon.
...
| Key | Type | Description | Environment override |
|---|---|---|---|
| `server.address` | string | TCP listen address for the HTTP API. | `VORNIK_SERVER_ADDRESS` |
| `server.read_timeout` | string | Max time to read a request. | — |
| `server.write_timeout` | string | Max time to write a response. | — |
| `server.unix_socket` | string | Optional Unix socket path the API is also served on, alongside the TCP address. Required for zero-egress agents. | `VORNIK_SERVER_UNIX_SOCKET` |
...
| Key | Type | Description | Environment override |
|---|---|---|---|
| `database.host` | string | PostgreSQL host. | `VORNIK_DATABASE_HOST` |
| `database.port` | int | PostgreSQL port. | `VORNIK_DATABASE_PORT` |
| `database.name` | string | Database name. | `VORNIK_DATABASE_NAME` |
| `database.user` | string | Database user. | `VORNIK_DATABASE_USER` |
| `database.password` | string | Password. Prefer the VORNIK_DATABASE_PASSWORD environment variable. | `VORNIK_DATABASE_PASSWORD` |
| `database.sslmode` | string | One of disable, require, verify-ca, verify-full. | `VORNIK_DATABASE_SSLMODE` |

## storage

| Key | Type | Description | Environment override |
|---|---|---|---|
| `storage.artifacts_path` | string | Local directory for artifacts when using the filesystem backend. | `VORNIK_ARTIFACTS_PATH` |
| `storage.backend` | string | filesystem (local disk) or s3 (S3-compatible object store, including MinIO). | `VORNIK_STORAGE_BACKEND` |
| `storage.s3.endpoint` | string | S3 endpoint URL. Leave empty for AWS S3; set for MinIO/non-AWS. | `VORNIK_STORAGE_S3_ENDPOINT` |
| `storage.s3.region` | string | AWS region. Required when backend is s3. | `VORNIK_STORAGE_S3_REGION` |
| `storage.s3.bucket` | string | Bucket holding artifacts. Required when backend is s3; must already exist. | `VORNIK_STORAGE_S3_BUCKET` |
| `storage.s3.prefix` | string | Optional key prefix to namespace one bucket across deployments. | `VORNIK_STORAGE_S3_PREFIX` |
| `storage.s3.access_key_id` | string | Access key. Prefer VORNIK_STORAGE_S3_ACCESS_KEY_ID. | `VORNIK_STORAGE_S3_ACCESS_KEY_ID` |
| `storage.s3.secret_access_key` | string | Secret key. Prefer VORNIK_STORAGE_S3_SECRET_ACCESS_KEY. | `VORNIK_STORAGE_S3_SECRET_ACCESS_KEY` |
| `storage.s3.use_path_style` | bool | Force path-style addressing. Set true for MinIO. | `VORNIK_STORAGE_S3_USE_PATH_STYLE` |
| `storage.s3.force_ssl` | bool | Require HTTPS. Set false only for local MinIO development. | `VORNIK_STORAGE_S3_FORCE_SSL` |

## artifacts

| Key | Type | Description | Environment override |
|---|---|---|---|
| `artifacts.artifacts_path` | string | Local directory for artifacts when using the filesystem backend. | `VORNIK_ARTIFACTS_PATH` |
| `artifacts.backend` | string | filesystem (local disk) or s3 (S3-compatible object store, including MinIO). | `VORNIK_STORAGE_BACKEND` |
| `artifacts.s3.endpoint` | string | S3 endpoint URL. Leave empty for AWS S3; set for MinIO/non-AWS. | `VORNIK_STORAGE_S3_ENDPOINT` |
| `artifacts.s3.region` | string | AWS region. Required when backend is s3. | `VORNIK_STORAGE_S3_REGION` |
| `artifacts.s3.bucket` | string | Bucket holding artifacts. Required when backend is s3; must already exist. | `VORNIK_STORAGE_S3_BUCKET` |
| `artifacts.s3.prefix` | string | Optional key prefix to namespace one bucket across deployments. | `VORNIK_STORAGE_S3_PREFIX` |
| `artifacts.s3.access_key_id` | string | Access key. Prefer VORNIK_STORAGE_S3_ACCESS_KEY_ID. | `VORNIK_STORAGE_S3_ACCESS_KEY_ID` |
| `artifacts.s3.secret_access_key` | string | Secret key. Prefer VORNIK_STORAGE_S3_SECRET_ACCESS_KEY. | `VORNIK_STORAGE_S3_SECRET_ACCESS_KEY` |
| `artifacts.s3.use_path_style` | bool | Force path-style addressing. Set true for MinIO. | `VORNIK_STORAGE_S3_USE_PATH_STYLE` |
| `artifacts.s3.force_ssl` | bool | Require HTTPS. Set false only for local MinIO development. | `VORNIK_STORAGE_S3_FORCE_SSL` |
...
| Key | Type | Description | Environment override |
|---|---|---|---|
| `metrics.enabled` | bool | Expose a Prometheus metrics endpoint. | `VORNIK_METRICS_ENABLED` |
| `metrics.addr` | string | Listen address for the dedicated metrics server. | `VORNIK_METRICS_ADDR` |
...
| Key | Type | Description | Environment override |
|---|---|---|---|
| `tracing.enabled` | bool | Enable OpenTelemetry tracing. | `VORNIK_TRACING_ENABLED` |
| `tracing.endpoint` | string | OTLP gRPC endpoint for trace export. | `VORNIK_TRACING_ENDPOINT` |

## logging

| Key | Type | Description | Environment override |
|---|---|---|---|
| `logging.level` | string | Log level (e.g. info, debug). | `VORNIK_LOG_LEVEL` |
...
| Key | Type | Description | Environment override |
|---|---|---|---|
| `gateway.enabled` | bool | Turn the query_api gateway integration on. | `VORNIK_GATEWAY_ENABLED` |
| `gateway.address` | string | Base URL the daemon uses to reach the local gateway, e.g. http://127.0.0.1:8010. | `VORNIK_GATEWAY_ADDRESS` |
| `gateway.token` | string | Internal daemon↔gateway key-auth secret (prefer token_file). | `VORNIK_GATEWAY_TOKEN` |
