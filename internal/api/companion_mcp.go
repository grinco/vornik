		...
		},
		{
			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"supersedes":       map[string]any{"type": "string", "description": "Answer a near-duplicate block: the id of the skill this one REPLACES. That skill is retired and its body preserved (readable by id); it is never overwritten. Use when the flagged match is the same knowledge, better expressed."},
					"confirm_distinct": map[string]any{"type": "string", "description": "Answer a near-duplicate block by asserting the skill is genuinely different: the value is the REQUIRED one-sentence justification, stored on the row and surfaced by skill_audit. Use when the flagged match has a different trigger. Do not pass a placeholder — this is the only record of why two similar skills both exist."},
				},
