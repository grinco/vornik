...
//
// The contrast with LookupActiveByHash above is deliberate: that one hides a
// revoked key because an auth layer must not let a caller tell "revoked" from
// "never existed". This one is the §5.4 claim path, where a revoked key must
// stay claimable — its recorded history is what the claim attributes to a
// person, and hiding the row would strand it.
...
// ListAttributable — mirrors the postgres method of the same name, including
// the prefix comparison rather than LIKE (TaskKeyNamePrefix ends in "_",
