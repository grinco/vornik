...
func TestResolveRegistryConfigDir(t *testing.T) {
	// Isolate from the operator's shell environment. The shell-env
	// fragment generated by the installer (88ca7b9ce) exports
	// VORNIK_CONFIGS_DIR pointing at the DEPLOYED config tree, and that
	// env override wins over every discovery heuristic in
	// resolveRegistryConfigDir — so without this the test asserts against
	// the developer's real ~/.config/vornik/configs and fails. CI has no
	// such export, which is why this only ever broke locally.
	t.Setenv("VORNIK_CONFIGS_DIR", "")
