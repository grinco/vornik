		...
		Sections: []Section{
			...
			},
			{
				...
				Fields: []Field{
					...
					{Path: "stage_child_artifacts_include", Label: "Stage child artifacts: include glob", Kind: KindString, Help: "Optional filter on stage_child_artifacts: stage only the children's artifacts whose name matches this glob (e.g. findings-*.md). Matched against the artifact's own name, not the staged <childID>- prefix. Empty stages every artifact of each child's latest completed execution. Use it to keep response transcripts out of an aggregating step's input. A child with no matching artifact is reported under inputArtifactsSummary.empty."},
					...
					{Path: "require_tools", Label: "Required tools", Kind: KindStringList, Help: "Tool contract: each named tool must complete SUCCESSFULLY at least once during the step, or it fails (one corrective retry). The sibling of require_output_glob — it exists because prose-mandatory is not mandatory: on 2026-08-25 an agent was told in its prompt that a Jira duplicate check was mandatory, the check 401'd, and the task still reported success. Names match the qualified mcp__server__tool form or the bare tool name, like allowedTools."},
					{Path: "auth_failure_mode", Label: "On connector auth failure", Kind: KindEnum, Enum: []string{"fail", "continue"}, Help: "Whether an auth-class (HTTP 401/403) tool failure fails this step. Default fail, and it needs no declaration — a connector that loses auth degrades every workflow that touches it. Choose continue ONLY for a step that deliberately degrades across connectors (try Slack, fall back to email). It opts out of ROUTING only: the failure is still recorded, still counted by the connector_auth doctor check, and still raises the operator alert."},
